...
documents_folder = os.path.join(home_directory, 'Documents', 'GamblingDataFile')
file_path = os.path.join(documents_folder, 'GamblingData.json')
default_variables = {
    "money": 100,
    "luck": 0,
    "cashout": 0,
...
def LoadVariables():
    if not os.path.exists(file_path) or os.path.getsize(file_path) == 0:
        SaveVariables(default_variables)
...
insert = 0
luck = Variables['luck']
cashout = Variables['cashout']
winprize = 0
loan = Variables['loan']
loanpayment = Variables['loanpayment']
loans = Variables['loans']
fired = Variables['fired']
...
def startgambling():
    ...
    Variables['luck'] = luck
    Variables['cashout'] = cashout
    Variables['loan'] = loan
    Variables['loanpayment'] = loanpayment
    Variables['loans'] = loans
    Variables['fired'] = fired
    ...
    if money > 0:
        ...
        if xyz == "cheatcode":
            ...
            if passs == "743596182":
                code = input()
                insert = '0' + ''.join(filter(lambda x: x.isdigit(), code))
                money += int(code)
                return startgambling()
            return startgambling()
        if xyz == "datafiles":
    ...
    if money > 0:
        ...
        def thebrokeassfunction():
            ...
            inpp = 0
            inppp = 0
            while inpp != "reset" and inpp != "loan" and inpp != "job" and inpp != "functions" and inpp != "cheatcode" and inpp != "datafiles":
                inpp = input()
            if inpp == "loan":
                if loans <= 4:
